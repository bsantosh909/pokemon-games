<template>
    <h1 class="text-emerald-500 text-3xl text-center mb-10">Name Pokemon by Types</h1>
    <div v-if="fetching">
    ...
    <div v-else>
        <div class="flex flex-wrap justify-center select-none">
            ...
            >
                <img :src="`/images/types/${type}.svg`" :alt="`Type ${type}`" class="w-6 h-6" />
                <span class="ml-2 capitalize text-gray-200 text-with-shadow"> {{ type }} Type </span>
            </button>
        </div>
        ...
        <div v-else class="bg-gray-100 p-3 mt-10">
            ...
            <div class="sticky top-10 bg-gray-100 py-1">
                <div>
                    <h3 class="text-center text-2xl font-semibold capitalize"> {{ activeType }} Type </h3>
                </div>
...
export default defineComponent({
    async setup() {
        ...
        const changeType = (gen: string) => {
            ...
            gameTime.value = pokemonList.value!.length <= 60 ? 480 : pokemonList.value!.length >= 100 ? 900 : 720;
        }
